...
{
    public class DepartmentsControllerIntTest
    {
        public DepartmentsControllerIntTest()
        {
        ...
        public async Task GetNonExistingDepartment()
        {
            ...
            var response = await _client.GetAsync("/api/departments/" + maxValue);
        ...
        public async Task UpdateDepartment()
        {
            ...
            DepartmentDto updatedDepartmentDto = _mapper.Map<DepartmentDto>(updatedDepartment);
            var response = await _client.PutAsync($"/api/departments/{_department.Id}", TestUtil.ToJsonContent(updatedDepartmentDto));
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        public async Task UpdateNonExistingDepartment()
        {
            ...
            DepartmentDto _departmentDto = _mapper.Map<DepartmentDto>(_department);
            var response = await _client.PutAsync("/api/departments/1", TestUtil.ToJsonContent(_departmentDto));
            response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
