...
{
    public class RegionsControllerIntTest
    {
        public RegionsControllerIntTest()
        {
        ...
        public async Task GetNonExistingRegion()
        {
            ...
            var response = await _client.GetAsync("/api/regions/" + maxValue);
        ...
        public async Task UpdateRegion()
        {
            ...
            RegionDto updatedRegionDto = _mapper.Map<RegionDto>(updatedRegion);
            var response = await _client.PutAsync($"/api/regions/{_region.Id}", TestUtil.ToJsonContent(updatedRegionDto));
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        ...
        public async Task UpdateNonExistingRegion()
        {
            ...
            RegionDto _regionDto = _mapper.Map<RegionDto>(_region);
            var response = await _client.PutAsync("/api/regions/1", TestUtil.ToJsonContent(_regionDto));
