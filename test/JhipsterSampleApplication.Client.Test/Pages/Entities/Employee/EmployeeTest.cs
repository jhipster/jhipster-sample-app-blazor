...
public class EmployeeTest : TestContext
{
    ...
    public void Should_DisplayAllEmployees_When_EmployeesArePresent()
    {
        ...
        var employeePage = RenderComponent<JhipsterSampleApplication.Client.Pages.Entities.Employee.Employee>();
    ...
    [Fact]
    public async Task Should_DeleteEmployee_WhenDeleteButtonClicked()
    {
        //Arrange
        var employees = _fixture.CreateMany<EmployeeModel>(10);
        _employeeService.Setup(service => service.GetAll()).Returns(Task.FromResult(employees.ToList() as IList<EmployeeModel>));
        ...
        var employeePage = RenderComponent<JhipsterSampleApplication.Client.Pages.Entities.Employee.Employee>(ComponentParameterFactory.CascadingValue(_modalService.Object));

        // Act
        var employeeToDelete = employees.First();

        // Assert
        await employeePage.Find("td>div>button").ClickAsync(new MouseEventArgs());
