...
public class JobHistoryTest : TestContext
{
    ...
    public void Should_DisplayAllJobHistories_When_JobHistoriesArePresent()
    {
        ...
        var jobHistoryPage = RenderComponent<JhipsterSampleApplication.Client.Pages.Entities.JobHistory.JobHistory>();
    ...
    [Fact]
    public async Task Should_DeleteJobHistory_WhenDeleteButtonClicked()
    {
        //Arrange
        var jobHistories = _fixture.CreateMany<JobHistoryModel>(10);
        _jobHistoryService.Setup(service => service.GetAll()).Returns(Task.FromResult(jobHistories.ToList() as IList<JobHistoryModel>));
        ...
        var jobHistoryPage = RenderComponent<JhipsterSampleApplication.Client.Pages.Entities.JobHistory.JobHistory>(ComponentParameterFactory.CascadingValue(_modalService.Object));

        // Act
        var jobHistoryToDelete = jobHistories.First();

        // Assert
        await jobHistoryPage.Find("td>div>button").ClickAsync(new MouseEventArgs());
